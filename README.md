...
| Notebook | Purpose |
|---|---|
| `01_data_mapping_and_merging.ipynb` | Maps parties to blocs, loads source files, creates targets, merges data, and performs quality checks |
| `02_eda_and_preprocessing.ipynb` | Performs EDA, chronological splitting, feature filtering, feature selection, and locality-type engineering |
| `03_modeling.ipynb` | Trains and compares baseline, XGBoost, CLR, and segmented models |
| `04_model_evaluation.ipynb` | Evaluates the selected model, creates visualizations, and analyzes errors |
...
- Google Drive when running the current Colab-based workflow

### Clone the Repository

```bash
git clone https://github.com/IfatDav/Election_Prediction_Project.git
...
The current notebooks use the following default Google Drive directory:

```text
/content/drive/MyDrive/לימודים/פרויקט DS
```

When using another path, update the following variable near the beginning of each notebook:
...
1. `01_data_mapping_and_merging.ipynb`
2. `02_eda_and_preprocessing.ipynb`
3. `03_modeling.ipynb`
4. `04_model_evaluation.ipynb`

Each notebook should be executed from top to bottom using **Run all**.
...
processed_data/election_targets_by_locality.csv
processed_data/coverage_by_election.csv
```
...
The project uses aggregated locality-level public data.
...
- Large raw datasets that are not required for understanding the project

Sensitive or local configuration files should be excluded using `.gitignore`.
